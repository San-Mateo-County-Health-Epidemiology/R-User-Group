---
format: gfm
title: "Map in R using ggplot2 package"
author: "Julie Bartels"
date: "2026-04-24"
---

The **sf** and **ggplot2** R libraries allow flexible and easy preparation of choropleth maps in R, which can be used in publications or reports. 

This tutorial explains how to:

1. Download shape files for ZIP codes and census tracts using the **tidycensus** package
2. Join rate/count data to shapefiles
3. Map incidence rates by ZIP code
4. Aggregate smaller geographic areas up into custom regions for mapping
5. Plot maps together for publications or reports
...
First, create sample incidence rate data by ZIP code for San Mateo County.
...
Next, use the **tidycensus** package to get shape files for the geometry of SMC ZIP codes to use for mapping. 
...
Note that you can also get shapefiles by census tract using the **tidycensus** package, as shown below:
...
For this map, break the continuous data (incidence rates) into discrete categories for cleaner mapping, which can be done using the *cut()* function. 
...
Use **ggplot** and the *geom_sf()* function to map incidence rates by ZIP code. 
...
Often, we want to visualize our data not by ZIP code or census tract, but by custom regions. Here, we will visualize sample incidence rate data by region. 
...
Use the *st_union()* function to aggregate ZIP code geometries into custom geographic regions. 
...
Use **gplot** and the *geom_sf()* function to map incidence rates by region. Note that this map uses a continuous scale instead of a discrete scale. 
...
For publications, reports, or presentations, you may want to plot maps (or other figures) into a cleanly-formatted grid. Use the **cowplot** package to do this. Here is an example of how to plot the two maps we created above together using the *plot_grid()* function.  
...
Use the ggsave function to export plots as png files that you can insert into reports.

(Check out the *here* package tutorial (https://github.com/San-Mateo-County-Health-Epidemiology/R-User-Group/blob/main/quarto-markdowns/here_package.md) for more information about directories for saving files)
