...
The HPI calculates a main HPI score, as well as scores for each of the domains. Here, we create indicator and year vectors that are relevant to those scores, as labeled throughout the following code chunk.

The user will need to input their api_key here as well. Note that the URL here is the same as the HPI score one.
