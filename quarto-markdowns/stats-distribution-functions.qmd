---
format: gfm
title: "`stats` distribution functions"
author: "Beth Jump"
date: "2025-07-24"
execute: 
  warning: false
...
R was originally created for statisticians and there are some very handy functions in the `stats` package which is included with the Base R installation.

Here we'll go through four functions that you can use with normal distributions. There are corollaries of these functions for other distributions including: binomial, uniform, exponential, gamma, F, Cauchy, etc. The functions are: `r_()`, `p_()`, `q_()` and `d()` (ex: `rnorm()`, `pnorm()`, `qnorm()` and `dnorm()` for the normal distribution).

A full list of functions is on the [`stats` package website](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/00Index.html). 
...
If your data need to have a certain shape, using `r()` (or one of the `r_()` functions) is likely a better option than trying to play around with the parameters of `sample()`. 
...
If you look at this graphically, it's the probability that your value falls on this line:
...
Here is a chart of 100,000 trials of the number of heads (or tails) in 10 flips of a coin:
...
Here we might want to know the probability of getting 4 heads in 10 flips. That probability is calculated with dbinom:
```{r}
dbinom(4, 10,  0.5)
```
