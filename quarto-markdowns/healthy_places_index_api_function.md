...
https://map.healthyplacesindex.org/?redirect=false. Click **Sign In** in
the upper right hand corner to create an account.
...
The vectors below are hand-coded with values for each of the variables
we are pulling from the HPI.

Line 52 (“api_key”) requires the user to input their own API key as a
character there.
...
Here we’ll loop through the variables saved above. We’ll save the year
...
Then we can do some cleaning to remove unnecessary variables and add new
ones:
