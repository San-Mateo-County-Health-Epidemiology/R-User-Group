...
* To expand your axis without specifying a specific range, use the `expand` argument in `scale_x_...()`. For example, `scale_x_continuous(expand = expansion(mult = c(0, 0.25)))` will expand the x axis 25% and `scale_x_continuous(expand = expansion(add = c(0,0.2)))` will add 0.2 x units to the right side of the plot.
